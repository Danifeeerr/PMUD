...
.on('reset', function (err, params, callback) {
  ...
  task_model.reset()
  .then(() => {callback(err, "Task reset");});
})

.on('system.listMethods', function (err, params, callback) {
  console.log("Method call params for 'system.listMethods': " + params);
  callback(err, ['count', 'getAll', 'get', 'add', 'update', 'delete', 'reset']);
})

.on('system.methodHelp', function (err, params, callback) {
...
.on('system.methodSignature', function (err, params, callback) {
